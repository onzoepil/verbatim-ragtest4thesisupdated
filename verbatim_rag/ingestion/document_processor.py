...
class DocumentProcessor:
    ...
    def process_url(
        ...
        content_md = self._normalize_for_rag(result.document.export_to_markdown())
    ...
    def extract_content_from_url(self, url: str) -> str:
        ...
        """
        result = self.converter.convert(url)
        return self._normalize_for_rag(result.document.export_to_markdown())
    ...
    def _normalize_for_rag(self, text: str) -> str:
        """Normalize extracted text so retrieval/extraction/highlighting share stable text form."""
        normalized = unicodedata.normalize("NFKC", text)

        # Remove private-use glyphs that can appear in PDF extraction
        normalized = re.sub(r"[\uE000-\uF8FF]", "", normalized)

        # De-hyphenate wrapped words: "implemen-\ntation" -> "implementation"
        normalized = re.sub(r"(?<=\w)-\s*\n\s*(?=\w)", "", normalized)

        # Normalize line endings first
        normalized = normalized.replace("\r\n", "\n").replace("\r", "\n")

        # Join single line breaks, preserve paragraph breaks
        normalized = re.sub(r"(?<!\n)\n(?!\n)", " ", normalized)
